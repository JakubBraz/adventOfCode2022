def find_next_step(board, max_row, max_col):
    next_step = {'R': {}, 'L': {}, 'D': {}, 'U': {}}
    for r in range(len(board)):
        for c in range(len(board[r])):
            if board[r][c] == '.':
                # print('ROW COL', r, c)
                nr, nc = (r, c + 1) if board[r][c + 1] != ' ' else (r, get_first_in_row(board, r))
                if board[nr][nc] == '.':
                    next_step['R'][(r, c)] = (r, nc)
                elif board[nr][nc] == '#':
                    next_step['R'][(r, c)] = (r, c)

                nr, nc = (r, c - 1) if board[r][c - 1] != ' ' else (r, get_last_in_row(board, r))
                if board[nr][nc] == '.':
                    next_step['L'][(r, c)] = (r, nc)
                elif board[nr][nc] == '#':
                    next_step['L'][(r, c)] = (r, c)

                nr, nc = (r + 1, c) if (c < len(board[r + 1]) and board[r + 1][c] != ' ') else (get_first_in_col(board, c), c)
                if board[nr][nc] == '.':
                    next_step['D'][(r, c)] = (nr, c)
                elif board[nr][nc] == '#':
                    next_step['D'][(r, c)] = (r, c)

                nr, nc = (r - 1, c) if (c < len(board[r - 1]) and board[r - 1][c] != ' ') else (get_last_in_col(board, c, max_row), c)
                if board[nr][nc] == '.':
                    next_step['U'][(r, c)] = (nr, c)
                elif board[nr][nc] == '#':
                    next_step['U'][(r, c)] = (r, c)
    return next_step


def get_first_in_row(board, r):
    result = [i for i, x in enumerate(board[r]) if x != ' ']
    return result[0]


def get_last_in_row(board, r):
    result = list(enumerate(board[r]))
    return result[-2][0]


def get_first_in_col(board, col):
    result = [i for i, row in enumerate(board) if col < len(row) and row[col] != ' ']
    return result[0]


def get_last_in_col(board, col, max_row):
    for r in range(max_row - 1, -1, -1):
        if col < len(board[r]) and board[r][col] != ' ':
            return r


def get_start_position(board):
    c = [i for i, x in enumerate(board[1]) if x == '.'][0]
    return (1, c)


def rotate(direction, rotation):
    if direction == "R":
        return "U" if rotation == "L" else "D"
    if direction == "L":
        return "D" if rotation == "L" else "U"
    if direction == "D":
        return "R" if rotation == "L" else "L"
    if direction == "U":
        return "L" if rotation == "L" else "R"


def go_steps(row, col, direction, next_move, moves):
    for m in moves:
        if type(m) == int:
            for s in range(m):
                row, col = next_move[direction][(row, col)]
        else:
            direction = rotate(direction, m)
    return row, col, direction


def go_steps2(row, col, direction, next_move, moves):
    for m in moves:
        if type(m) == int:
            for s in range(m):
                row, col, direction = next_move[direction][(row, col)]
        else:
            direction = rotate(direction, m)
    return row, col, direction


def facing(direction):
    if direction == "R":
        return 0
    if direction == "D":
        return 1
    if direction == "L":
        return 2
    if direction == "U":
        return 3


def mark_walls(board, size):
    current_wall = 0
    result = {}
    for init_row in range(1, size * 5, size):
        for init_col in range(1, size * 5, size):
            if init_row < len(board) and init_col < len(board[init_row]) and board[init_row][init_col] != ' ':
                current_wall += 1
                for row in range(init_row, init_row + size):
                    for col in range(init_col, init_col + size):
                        if board[row][col] == '.':
                            result[(row, col)] = current_wall, ((init_row + size) // size, (init_col + size) // size)
    return result


def local_to_global(row, col, cube_row, cube_col, size):
    return row + (cube_row - 1) * size, col + (cube_col - 1) * size


def global_to_local(row, col, size):
    return (row - 1) % size + 1, (col - 1) % size + 1


def get_new_positions(r, c, direction, wall_marked, wall_mapping, size):
    _cube, (cube_row, cube_col) = wall_marked[(r, c)]
    (new_cube_row, new_cube_col), f, new_dir = wall_mapping[((cube_row, cube_col), direction)]
    r2, c2 = global_to_local(r, c, size)
    new_local_row, new_local_col = f((r2, c2))
    new_global_row, new_global_col = local_to_global(new_local_row, new_local_col, new_cube_row, new_cube_col, size)
    return new_global_row, new_global_col, new_dir, r2, c2


def find_next_step2(board, max_row, max_col, wall_marked, wall_mapping, size):
    next_step = {'R': {}, 'L': {}, 'D': {}, 'U': {}}
    for r in range(len(board)):
        for c in range(len(board[r])):
            if board[r][c] == '.':
                new_r, new_c, new_dir, old_lr, old_lc = get_new_positions(r, c, "R", wall_marked, wall_mapping, size)
                nr, nc, nd = (r, c + 1, "R") if old_lc + 1 <= size and board[r][c + 1] != ' ' else (new_r, new_c, new_dir)
                if board[nr][nc] == '.':
                    next_step['R'][(r, c)] = (nr, nc, nd)
                elif board[nr][nc] == '#':
                    next_step['R'][(r, c)] = (r, c, "R")

                new_r, new_c, new_dir, old_lr, old_lc = get_new_positions(r, c, "L", wall_marked, wall_mapping, size)
                nr, nc, nd = (r, c - 1, "L") if old_lc - 1 >= 1 and board[r][c - 1] != ' ' else (new_r, new_c, new_dir)
                if board[nr][nc] == '.':
                    next_step['L'][(r, c)] = (nr, nc, nd)
                elif board[nr][nc] == '#':
                    next_step['L'][(r, c)] = (r, c, "L")

                new_r, new_c, new_dir, old_lr, old_lc = get_new_positions(r, c, "D", wall_marked, wall_mapping, size)
                nr, nc, nd = (r + 1, c, "D") if (old_lr + 1 <= size and board[r + 1][c] != ' ') else (new_r, new_c, new_dir)
                if board[nr][nc] == '.':
                    next_step['D'][(r, c)] = (nr, nc, nd)
                elif board[nr][nc] == '#':
                    next_step['D'][(r, c)] = (r, c, "D")

                new_r, new_c, new_dir, old_lr, old_lc = get_new_positions(r, c, "U", wall_marked, wall_mapping, size)
                nr, nc, nd = (r - 1, c, "U") if (old_lr - 1 >= 1 and board[r - 1][c] != ' ') else (new_r, new_c, new_dir)
                if board[nr][nc] == '.':
                    next_step['U'][(r, c)] = (nr, nc, nd)
                elif board[nr][nc] == '#':
                    next_step['U'][(r, c)] = (r, c, "U")
    return next_step


def main():
    # (board, moves, max_row, max_col), size = parse_input(day_input_test()), 4
    (board, moves, max_row, max_col), size = parse_input(day_input()), 50
    # print("LEN", len(board))
    # print("LEN", len(board[0]))
    # print(moves)
    next_steps = find_next_step(board, max_row, max_col)
    # print(next_steps['R'])
    # print(next_steps['L'])
    # print(next_steps['D'])
    # print(next_steps['U'])
    start_row, start_col = get_start_position(board)
    # print(start_row, start_col)
    final_row, final_col, direction = go_steps(start_row, start_col, "R", next_steps, moves)
    # print(final_row, final_col, direction)
    result1 = final_row * 1000 + final_col * 4 + facing(direction)
    print(result1)

    marked = mark_walls(board, size)
    # print(marked)
    # print(len(next_steps['R']))
    # print(len(marked))
    wall_mapping = {
        ((1, 3), "R"): ((3, 4), lambda x: (size, size + 1 - x[0]), "L"),
        ((1, 3), "L"): ((2, 2), lambda x: (1, x[0]), "D"),
        ((1, 3), "D"): ((2, 3), lambda x: (1, x[1]), "D"),
        ((1, 3), "U"): ((2, 1), lambda x: (1, size + 1 - x[1]), "D"),

        ((2, 1), "R"): ((2, 2), lambda x: (x[0], 1), "R"),
        ((2, 1), "L"): ((3, 4), lambda x: (size, size + 1 - x[0]), "U"),
        ((2, 1), "D"): ((3, 3), lambda x: (size, size + 1 - x[1]), "U"),
        ((2, 1), "U"): ((1, 3), lambda x: (1, size + 1 - x[1]), "D"),

        ((2, 2), "R"): ((2, 3), lambda x: (x[0], 1), "R"),
        ((2, 2), "L"): ((2, 1), lambda x: (x[0], size), "L"),
        ((2, 2), "D"): ((3, 3), lambda x: (size + 1 - x[1], 1), "R"),
        ((2, 2), "U"): ((1, 3), lambda x: (x[1], 1), "R"),

        ((2, 3), "R"): ((3, 4), lambda x: (1, size + 1 - x[0]), "D"),
        ((2, 3), "L"): ((2, 2), lambda x: (x[0], size), "L"),
        ((2, 3), "D"): ((3, 3), lambda x: (1, x[1]), "D"),
        ((2, 3), "U"): ((1, 3), lambda x: (size, x[1]), "U"),

        ((3, 3), "R"): ((3, 4), lambda x: (x[0], 1), "R"),
        ((3, 3), "L"): ((2, 2), lambda x: (size, size + 1 - x[0]), "U"),
        ((3, 3), "D"): ((2, 1), lambda x: (size, size + 1 - x[1]), "U"),
        ((3, 3), "U"): ((2, 3), lambda x: (size, x[1]), "R"),

        ((3, 4), "R"): ((1, 3), lambda x: (size + 1 - x[0], size), "L"),
        ((3, 4), "L"): ((3, 3), lambda x: (x[0], size), "L"),
        ((3, 4), "D"): ((2, 1), lambda x: (size + 1 - x[1], 1), "R"),
        ((3, 4), "U"): ((2, 3), lambda x: (size + 1 - x[1], size), "L"),
    }
    wall_mapping2 = {
        ((1, 2), "R"): ((1, 3), lambda x: (x[0], 1), "R"),
        ((1, 2), "L"): ((3, 1), lambda x: (size + 1 - x[0], 1), "R"),
        ((1, 2), "D"): ((2, 2), lambda x: (1, x[1]), "D"),
        ((1, 2), "U"): ((4, 1), lambda x: (x[1], 1), "R"),

        ((1, 3), "R"): ((3, 2), lambda x: (size + 1 - x[0], size), "L"),
        ((1, 3), "L"): ((1, 2), lambda x: (x[0], size), "L"),
        ((1, 3), "D"): ((2, 2), lambda x: (x[1], size), "L"),
        ((1, 3), "U"): ((4, 1), lambda x: (size, x[1]), "U"),

        ((2, 2), "R"): ((1, 3), lambda x: (size, x[0]), "U"),
        ((2, 2), "L"): ((3, 1), lambda x: (1, x[0]), "D"),
        ((2, 2), "D"): ((3, 2), lambda x: (1, x[1]), "D"),
        ((2, 2), "U"): ((1, 2), lambda x: (size, x[1]), "U"),

        ((3, 1), "R"): ((3, 2), lambda x: (x[0], 1), "R"),
        ((3, 1), "L"): ((1, 2), lambda x: (size + 1 - x[0], 1), "R"),
        ((3, 1), "D"): ((4, 1), lambda x: (1, x[1]), "D"),
        ((3, 1), "U"): ((2, 2), lambda x: (x[1], 1), "R"),

        ((3, 2), "R"): ((1, 3), lambda x: (size + 1 - x[0], size), "L"),
        ((3, 2), "L"): ((3, 1), lambda x: (x[0], size), "L"),
        ((3, 2), "D"): ((4, 1), lambda x: (x[1], size), "L"),
        ((3, 2), "U"): ((2, 2), lambda x: (size, x[1]), "U"),

        ((4, 1), "R"): ((3, 2), lambda x: (size, x[0]), "U"),
        ((4, 1), "L"): ((1, 2), lambda x: (1, x[0]), "D"),
        ((4, 1), "D"): ((1, 3), lambda x: (1, x[1]), "D"),
        ((4, 1), "U"): ((3, 1), lambda x: (size, x[1]), "U"),
    }
    next_steps2 = find_next_step2(board, max_row, max_col, marked, wall_mapping2, size)
    # print(next_steps2)
    final_row, final_col, direction = go_steps2(start_row, start_col, "R", next_steps2, moves)
    # print(final_row, final_col, direction)
    result2 = final_row * 1000 + final_col * 4 + facing(direction)
    print(result2)


def parse_input(arg):
    result = arg.split('\n\n')
    moves = parse_moves(result[1])
    map = result[0].split('\n')
    max_row = len(map) + 2
    max_col = max((r for r in map), key=lambda x: len(x))
    max_col = len(max_col) + 2
    map = [' ' + row + ' ' for row in map]
    extra_row = ' ' * (max_col+2)
    map = [extra_row] + map + [extra_row]
    return map, moves, max_row, max_col


def parse_moves(moves):
    element = ''
    result = []
    for c in moves:
        if c.isnumeric():
            element += c
        else:
            result.extend([c] if not element else [int(element), c])
            element = ''
    if element:
        result.append(int(element))
    return result


def day_input_test():
    return """        ...#
        .#..
        #...
        ....
...#.......#
........#...
..#....#....
..........#.
        ...#....
        .....#..
        .#......
        ......#.

10R5L5R10L4R5L5"""


def day_input():
    return """                                                  ..................#....#....#...#...#................#.................#..............#....#......#.
                                                  .#.......................#.......#.......#.............#.#...#....#....#.....#.#.......#.....#......
                                                  #......#...#...##....#.......................#.#.......#..#........#....#......................#....
                                                  ..#......#.....#.......................#...............#....#..........#........#...................
                                                  .....#....#.#......#..#.....#....#...............................#.#......#..###..............#.....
                                                  #...#.#..#.##......#...............#..........###...#......#.............#...........###.....#.....#
                                                  ......#......#......#..........#.......#...#....#...............#.......................#...........
                                                  #......#...........#.#...............#....#..#..#.#..............#........#.....#...#..#.#..........
                                                  ..............................##.........................................#..........................
                                                  ##......#........................#..............#.........#..##..................#.#......#...#.....
                                                  ...#..#.#............#.....#.....#.#.#.#.##......#......................................#.......#...
                                                  ..#................#................#..........#.......................#...#..#.......#.....#.......
                                                  ...................#.#........................#........................................##...#.....#.
                                                  .............#..#......#.................#.#.......#................#.#..........#..........#....#..
                                                  ................#..#..........#.#.............#......#....#..............#...........##.#....#..#...
                                                  ........................#.........#......#.......#...#...........#...........#...................##.
                                                  ............#.#.......#............#.......#.#..............#..............#......#.#.........#.....
                                                  .#.........................#.#.............#...#.......##....#.##..#...##...............#.......#.#.
                                                  ................#..................#....##........#.#..#.....#.........................#........#...
                                                  .....#........#.....#...#..........................#.......#...#..#...##.....#.#......#.............
                                                  .................#..#....#.....#........#...........................................#.....#...#.##..
                                                  ......................#...................#...#...#.......#..#.....#......#......#.#.....#......#...
                                                  ...#..........#...#......#.........#..##............#............#.................#.....#......#...
                                                  .........................#.#.....#...#.............#..#..###..#.#....#..#........#.##...............
                                                  .............#.........#...#..#.......#..........#.............#............#..#..#.........#......#
                                                  ......#.#..#.........#.....#..#....#............#.......#.#........#.................#...#.........#
                                                  .....................#...................#...#...........#.###..#..............##........#......#...
                                                  .......#.............#........##......#..........#..##........#..#..#.....#........#......#.........
                                                  #.........#.......#.........#........#...................#..#..#............#.#...#.................
                                                  .......#.......#........#.....#..........................#........#.........#........#..............
                                                  ..................#.....##.....#..#.............##....#........#..#..#............#.............#...
                                                  ..................#.........#....#.#.#.....#......#.......#...#......##..........#.......#..........
                                                  .#...#....#...##.............#..###.....#.........##.........#.....#.....#.#...#....................
                                                  ..#.........#.....#.........#.................##.......#............#...............................
                                                  #................#...................##.#....#......#.........#..........................#.....#....
                                                  .................#........................#....#............#...#........#.....................#....
                                                  ...............#...............................................#..#....................#.#..........
                                                  #............#...#.................#..#................#.............#.....#..............#....#....
                                                  .................#....#...............#.......#.........#...##..............#.....#..#..............
                                                  .#..............................#..#.......#.........................................#........#.....
                                                  .......#........#.........#...#..................................................#........###....##.
                                                  ......#.........#..........#........###...............#.#......#.#..........#........#...#.....#....
                                                  #.#..................#............#.#...#.....................#..##..#...........##.................
                                                  ...#..................#..................#....#....#........#......#....................#..#.......#
                                                  ..........................#.....#..#..#.....#...#...##..#...#.................#..#......#..........#
                                                  ....................#.....................#.......#.....#.........#...##.##..........#.........#....
                                                  ..................#.........#.........#...##.........#.#..#.....#....#........................#....#
                                                  #..##......#...#.....#......#.....#..#............#####...#....#.....##.#...............##..#.......
                                                  ....#.........#.....#.....#.....#.....................................#...........##.......##.......
                                                  ......##......#...##...............................#................#................#..............
                                                  ..#....#...........#.........................##.#.
                                                  .............#.................##..#.......#.....#
                                                  .............#.......##......................#....
                                                  ....................#..#.#.................#......
                                                  .............#...........#......#.....#.....#..#..
                                                  #.................................................
                                                  .............................................#....
                                                  .........#........................................
                                                  ........#.#.................................#.....
                                                  .....#...#.............#................#.........
                                                  #........#..#......#...........#....#.....##..#...
                                                  ..........#.................................#.....
                                                  ...........##...................................#.
                                                  ....##....#....................#...#.........#...#
                                                  .#....#..........................#................
                                                  ....##......#.....................................
                                                  ..........#.......#.......##...#.........#..#.....
                                                  ................##.................##.....#.#....#
                                                  .........................................#........
                                                  ..................#...............#...............
                                                  .#.......#.....##.......#....##.....#.....#.#.....
                                                  #.........#...#...#.........#......#..#...........
                                                  #.......#......#.........#.....................#..
                                                  #.......#.....#....#..............................
                                                  #....#...#.#......................................
                                                  .............#.....................#.....#....#...
                                                  ..#..........#..................#................#
                                                  ......#................#.........###.#............
                                                  ....#...............................#....#........
                                                  ........................#.........................
                                                  #.#..#.#.......................#..................
                                                  #.#........#.......#.#......#.##.#......#.#...#...
                                                  ..#.......................#.#.......#.............
                                                  ..................................................
                                                  ........#.....###.........#.....#....#.#...##.....
                                                  .......#......#.......#.#........#............#...
                                                  ..##..............#.....#.#...........#...........
                                                  ..............##.#.........#.......#..........#...
                                                  ..#...........#............#......................
                                                  .......................#...#.#...#.#.....#..#..#..
                                                  ............#..............................#......
                                                  ......#.............#.##..#..#...............#....
                                                  ...#.........................................#....
                                                  .#........#.....#.....#..................#....#...
                                                  #......#.....................#..#........#........
                                                  ..#.....................##..........#...#.....#...
                                                  .......#.#....#......#.....#.....................#
                                                  .......................#...........##.........#...
                                                  .....##..........#......#.........................
                                                  ..........#.............#.............#........#..
..........................#..#....#...#...#................##....................#......#...#.....#.
.................................#........#.#......#..#...#...............................#.........
...............#.##............#.##.#.#...........#.............#..................#....#...........
#.........................#......#........#.........#......#...............#.....#...##.........#...
.#..#.....#...............#..........#...............#.....#....##....#.............................
..#........................#..................#.......#.............#......#...##........#...#......
....#...#........#................##.........#..........#..........#....#....#......................
...#..............#.##.##.................#.....#...........#.................#............#.....#..
...#..#...................#..#....#...#....#.............#...#.......#.......#..........##.......#..
........................##.....#.......#..#......#..................#..#...#.............#..#...#...
#...............#.......#...........#...................#..................#......#..............##.
#..#....................#..............#.................#.#...#..#...#............#...#....#.......
............#...#..................#...........#................................................#...
.............#..........................#......#................................#..................#
.##...#..#....##...#.........#..#..........#.........#.....................#...#...........##..#..#.
.............#.............#........#.....#........#.....#.................#...#....................
............#............#................#................##........#......#.#....#...#......##....
....#.........#..............#...............#...#......#.......................#...#.......#.......
....#............................#.........................#.....#..........#...#.......#..#..#.....
..........#..#.......#........#....###......#........#............#.................................
........................#...#....#....#.##...#............#.....#............................#......
........#..#...#........................#...............................................#...........
....#....#.....#.....#......#....#.....#....#.#............................##................#......
..........#........................##............#.#..........#....#.................#.........#....
............#...#......#...........................#................#...##.............#..........##
..#...............#...#.....#..#.........#..#.................................#.......#.#.........#.
........##........#..................................#...#.....#....................................
..#....#.............#...#...................#........##.......#.......#...................#........
....................#...#........##.#..........#........#..................#........#.......#.......
....#...#.#....#..####..#...#.......................#.....#..........................#.#............
..#....#....#........#...........#..........##...............#....#.................................
#.#.............#...........#..........#.....#...#....#..#...........................#.....#........
.....#..............................................................##..............................
..............#...##.......#....#..#..#..........#.....#..#.......##...#........##..................
.................#...........................#.#.................................#.......#.......#..
#.....#....#.#......##.........................#.......#....##.#..##......#.....#...#...............
.......#..#......##...#......#.....##..#..........#.#.#...........#...#.................#....#...#..
#..#.....................#....#..#..............#.#................#............#................#..
....#.....#..............#........................#....#...#..#.....................................
..#........................#.....#..................#....#.##.#............#.........#...........#..
............##..#....#......#...........#.....#..............#...................................##.
..#........#......##...#.#..........#.....#..........#.......#.........#...................#........
........#.....................#...#..............#.....##...............................#.#......#..
......................#.................................#............#.......................##...#.
...#...................#..........#.....#.##....#.....#.......#.....#...............................
..........................................#...............#........#.........#................#..#..
...#.....#.#...............#...#...........#......#.................................................
....#......#.#......#.............#.##....#..................#.#..#.........#....#.#..#...........#.
#..........#..#............#...........#..............#............#..###......................#....
..............................#.........#...........#........#...............#.##...............#.#.
#.#........#.....#..#..#.#......#........#......#.
#..#.............#.........##.........#..#.....##.
........##.....#.#..............................#.
............................#...........#........#
...................................#..............
.........#......................#.......#..#.....#
##.............#........#.........................
......#.........#............#................##..
......#.......#.........##.......#..#..#..........
#......#..#.................#..###...#....#.......
....##..............................#..#........#.
......#.#.....#........#..#...#...................
..#......#.#.#........#.........#.................
........##............#.#.................#......#
.#..............#....#...............#.#..#..#.#..
...#................#...#.........................
........#..............#...#......................
.#.................#.....#..............#.........
.......#........................#.#..............#
..#..........#........#....#..........#..........#
........#.#.............#.#........#..............
.#...#........................#...................
.......#...........#.#.#.......##............#...#
....##...#........#................#..............
..............................#...............#...
.#............#.......#..............#............
.....#.........#..............#...................
........#..#...#....#.#.................#.........
.............#.....................#..............
.#....#..............#.....#......................
..#.#............#.............#..................
....#..........#....#.........#...#........##.....
............................#.#.#.........#.#.....
......#...............#.................#.#.......
..........#.........#...........#....###.......#..
........#.........#............#..#..#....##......
.............#..#....#........................#...
..........#.#......#.....................##.......
......###....#.......#..#.#......#..#..........#..
.......###............#.#.#............#..........
..#........#.........#.......#...................#
.........#.#.....#......................#.........
#....#..#.#..#.......###....#.#.................#.
......#.........#.#...............................
...#....#.........##..#.....#......#..#.........#.
.......#........##.......#..#....##....#....#.....
..#....#.........#.....#......##.................#
#.#.......#.....#...........#.................#..#
........#......#.........................#.#.....#
.......#...#....##...........#.#.......#..........

"""

if __name__ == "__main__":
    main()
